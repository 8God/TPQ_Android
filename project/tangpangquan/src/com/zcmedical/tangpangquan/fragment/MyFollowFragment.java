...
public class MyFollowFragment extends BaseListFragment<CircleMembersAdapter, UserEntity> {

    @Override
    protected void init() {
        super.init();

        setNoDataTextTip("您暂无关注任何人喔");
    }
    ...
    protected void fetchDataList(int offset, int page_sizes) {
        ...
        CommonRequest fetchMyFollowList = new CommonRequest();
        fetchMyFollowList.setRequestApiName(InterfaceConstant.API_USER_FOLLOW_FETCH);
        fetchMyFollowList.setRequestID(InterfaceConstant.REQUEST_ID_USER_FOLLOW_FETCH);
        fetchMyFollowList.addRequestParam(APIKey.FOLLOWER, userId);

        addRequestAsyncTask(contentView, fetchMyFollowList);
            ...
            if (status == APIKey.STATUS_SUCCESSFUL) {
                if (null != resultMap) {
                    ...
                    if (null != tmpUserList && tmpUserList.size() > 0) {
                        List<UserEntity> userList = EntityUtils.getMyFollowEntityList(tmpUserList);
                        if (null == dataList) {
