...
public class HotPostsListPager extends BasePager {
    ...
    public HotPostsListPager(Context context) {
        super(context);
    ...
    private void initLayout() {
        ...
        backToTopButtonParams.bottomMargin = DensityUtil.dip2px(getContext(), 60); // HomeActivity 底部tabbar的高度
    ...
    private void fetchPostsList(int offset) {
        Map<String, Object> postsListMap = TpqApplication.getInstance(getContext()).getHotPostsListMap();
        if (null != postsListMap) {
            toBeContinued = TypeUtil.getInteger(postsListMap.get(CommonConstant.KEY_TO_BE_CONTINUED), 0);
            Object listOj = postsListMap.get(CommonConstant.KEY_DATA_LIST);
            if (null != listOj && listOj instanceof List<?>) {
                postsList = (List<PostsEntity>) listOj;
                if (null != postsList && postsList.size() > 0) {
                    showPostsList();
                    ...
                    if (null != resultMap) {
                        ...
                        if (null != rawPostsList && rawPostsList.size() > 0) {
                            ...
                            if (null != tmpPostsList) {
                                ...
                                TpqApplication.getInstance(getContext()).setHotPostsList(postsList, toBeContinued);
                            }
                        }
                    }
    ...
    private void showPostsList() {
        ...
        if (null == postsAdapter) {
            postsAdapter = new HotPostsAdapter(getContext(), postsList);
