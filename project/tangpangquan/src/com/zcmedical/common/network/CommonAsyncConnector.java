...
import com.zcmedical.common.utils.FileUtil;
...
public class CommonAsyncConnector extends AsyncTask<CommonRequest, Integer, Map<String, Object>> {
    ...
    private static final String REQUEST_URL_DUMMAY_BASE_STRING = "http://" + TpqApplication.class.getPackage() + "/dummy";
    ...
    static {
        ...
        fileTypeParamNameList.add(APIKey.PIC_THREAD_PIC);
        fileTypeParamNameList.add(APIKey.THREADS_COMMENT_PIC);
        fileTypeParamNameList.add(APIKey.USER_USER_HEAD_PIC);
        fileTypeParamNameList.add(APIKey.MEDICAL_RECORD_PIC);
    }
    ...
    protected Map<String, Object> doInBackground(CommonRequest... requests) {
        ...
        try {
            if (null != notApplicationContext) {
                ...
                TpqApplication tpqApplication = TpqApplication.getInstance(notApplicationContext);

                if (isOnlineAvailable && networkInfo != null && networkInfo.isConnected()) {
                    tpqApplication.resetNetworkSettingsMessageEnabled();
                    result = sendByNetwork(requests[0]);
                } else if (IS_LOCAL_DATABANK_SUPPORTED) {

                    //                        TpqApplication.showNetworkSettingsMessage();
    ...
    protected void onPostExecute(Map<String, Object> result) {
        ...
        if (null == result && isOnlineAvailable && null != notApplicationContext) {
            TpqApplication.getInstance(notApplicationContext).showNetworkSettingsMessage();
    ...
    private String parseAuthResponseStr(final String authResponseStr) {
        ...
        //        try {
        //            responseStr = Des3Util.decrypt(authResponseStr, TpqApplication.getInstance(notApplicationContext).getUserPassword());
        //        } catch (Exception e) {
