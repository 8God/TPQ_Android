...
public class HXNotifier {
    ...
    public void viberateAndPlayTone(EMMessage message) {
        ...
        try {
            ...
            SharedPreferences spf = TpqApplication.getInstance().getSharedPreferences(TpqApplication.class.getName(), 0);
            // 判断是否处于静音模式
            if (audioManager.getRingerMode() == AudioManager.RINGER_MODE_SILENT || !spf.getBoolean("mPushSlideSwitchView4", AlarmReceiver.DEFAULT_STATUS)) {
                EMLog.e(TAG, "in slient mode now");
                return;
            } else {
                Log.d("@@", "in non-slient mode now");
            }
