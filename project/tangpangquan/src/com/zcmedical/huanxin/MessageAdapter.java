...
import com.zcmedical.tangpangquan.activity.AlertDialog;
import com.zcmedical.tangpangquan.activity.ChatActivity;
import com.zcmedical.tangpangquan.activity.ShowBigImage;
...
public class MessageAdapter extends BaseAdapter {
    ...
    private Doctor doctor = null;//from
    private AsyncHttpClient client;
    private SharedPreferences spf;

    public MessageAdapter(final Context context, String username, int chatType) {
        this.username = username;
        this.context = context;
        inflater = LayoutInflater.from(context);
        activity = (Activity) context;
        spf = TpqApplication.getInstance().getSharedPreferences(TpqApplication.class.getName(), 0);
        client = new AsyncHttpClient();
        RequestParams params = new RequestParams();
        params.add("id", username);
        if (BuildConfig.DEBUG) {
            Log.d(TAG, "doctor(id) : " + username);
        }
        client.post(InterfaceConstant.DOCTOR_FETCH, params, new JsonHttpResponseHandler() {
            ...
            public void onSuccess(int statusCode, Header[] headers, JSONObject response) {
                ...
                if (doctors.size() > 0) {
                    //Picasso.with(context).load(doctors.get(0).getHead_pic()).placeholder(R.drawable.default_avatar).into(holder.avatar);
                    doctor = doctors.get(0);
                    if(!TextUtils.isEmpty(doctor.getNickname())){
                        ((ChatActivity)context).setTitleName(doctor.getNickname());
                    }
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            ...
            if (message.getType() == EMMessage.Type.IMAGE) {
                ...
                // 语音通话及视频通话
                if (message.getBooleanAttribute(CommonConstant.MESSAGE_ATTR_IS_VOICE_CALL, false) || message.getBooleanAttribute(CommonConstant.MESSAGE_ATTR_IS_VIDEO_CALL, false)) {
        ...
        LinearLayout conversationL = (LinearLayout) convertView.findViewById(R.id.conversationL);
        if (position == 0) {
            ...
            if (prevMessage != null && DateUtils.isCloseEnough(message.getMsgTime(), prevMessage.getMsgTime())) {
                ...
                timestamp.setVisibility(View.GONE);
                if(conversationL!=null){
                    conversationL.setVisibility(View.GONE);
                }
            } else {
                timestamp.setText(DateUtils.getTimestampString(new Date(message.getMsgTime())));
                timestamp.setVisibility(View.VISIBLE);
                timestamp.setVisibility(View.GONE);
                if(conversationL!=null){
                    conversationL.setVisibility(View.VISIBLE);
                }
            }
        }
        return convertView;
    ...
    private void setUserAvatar(EMMessage message, ImageView imageView) {
        if (message.direct == Direct.SEND) {
            //显示自己头像
            if (TextUtils.isEmpty(spf.getString(APIKey.USER_HEAD_PIC, ""))) {
                UserUtils.setUserAvatar(context, EMChatManager.getInstance().getCurrentUser(), imageView);
            } else {
                Picasso.with(context).load(spf.getString(APIKey.USER_HEAD_PIC, "")).placeholder(R.drawable.default_avatar).into(imageView);
            }
        } else {
            if (doctor != null && !TextUtils.isEmpty(doctor.getHead_pic())) {
                Picasso.with(context).load(doctor.getHead_pic()).placeholder(R.drawable.default_avatar).into(imageView);
            } else {
        ...
        holder.tv.setOnLongClickListener(new OnLongClickListener() {
            ...
            public boolean onLongClick(View v) {
                activity.startActivityForResult(
                        (new Intent(activity, com.zcmedical.tangpangquan.activity.ContextMenu.class)).putExtra("position", position).putExtra("type", EMMessage.Type.TXT.ordinal()),
